im_trans = imread('HG13.jpg');    % load the original image
org_gray = rgb2gray(im_trans);
im_org = imread('HG2.jpg');   % load the zoomed in and rotated image
tran_gray = rgb2gray(im_org);

p_org = detectSIFTFeatures(org_gray);   % detect the feature points on the 
                                        % on the original image.
p_org = selectStrongest(p_org, 100);    % select the strongest 100 points

p_tran = detectSIFTFeatures(tran_gray); % detect the feature points on the 
                                        % translated image. 
...
tform = estimateGeometricTransform(matchedPoints1.Location, matchedPoints2.Location,...
    'projective', 'MaxNumTrials', 5000);
